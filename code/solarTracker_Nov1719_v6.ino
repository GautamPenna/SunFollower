/* Name:        Sun Follower
   ...
   Motors:
   =======
   NEMA 17 is Altitude Motor, NEMA 23 is the Azimuth Motor. I'm use TB6600 as the Stepper Driver.
...
//GPS Definitions
SoftwareSerial mySerial(50, 52); // Setting up the new serial port to communicate to the GPS tracker
...
const int Vtracker = A4; // Using analog pin 4 for tracker
const int Vfixed = A5; //Using analog pin 5 for fixed

File dataCAP; // File Handle
const int csPIN = 53; //Arduino MEGA SD card PIN
...
void setup() {
  ...
  pinMode(csPIN , OUTPUT); // csPin in SD card set as an OUTPUT because we write to a file

  //Voltage Readings
  pinMode (Vfixed, INPUT); //defining the pin for Fixed Read
  pinMode (Vtracker, INPUT); //defining the pin for Tracing Read
  ...
  String FName = String(String (gpsYEAR) + String (gpsMON) + String (gpsDAY) + ".csv");
...
void loop() {
  ...
  if (inRealMode) {
    ...
    while (localHOUR < 21) {
      ...
      /// Right before looping back to localHOUR < 21. we will print power, voltage and current to SD card
      V2a = analogRead(Vtracker);
      V1a = vBridge(V2a , R1a , R2a);
      Ia = V2a / (R2a * 1000);
      Pa = V1a * Ia;

      V2b = analogRead (Vfixed);
      V1b = vBridge(V2b, R1b, R2b);
      Ib = V2b / (R2b * 1000);
      Pb = V1b * Ib;
      ...
      dataCAP = SD.open(FName, FILE_WRITE);
      // Date Time Fixed fV fI alt azi sV sI

      //dataCAP.print("DATE : ");
      dataCAP.print(gpsMON); dataCAP.print("/"); dataCAP.print(gpsDAY); dataCAP.print("/"); dataCAP.print(gpsYEAR); dataCAP.print(",");
      //dataCAP.print("TIME : ");
      dataCAP.print(gpsHOUR); dataCAP.print(":"); dataCAP.print(gpsMIN); dataCAP.print(",");
      //dataCAP.print("Fixed Panel Voltage : ");
      dataCAP.print(V1b);  dataCAP.print(",");
      //dataCAP.print("Fixed Panel Current : ");
      dataCAP.print(Ib); dataCAP.print(",");
      //dataCAP.print("Fixed Panel Power   : ");
      dataCAP.print(Pb);   dataCAP.print(",");
      //dataCAP.print("Solar tracker Voltage : ");
      dataCAP.print(V1a);  dataCAP.print(",");
      //dataCAP.print("Solar tracker Current : ");
      dataCAP.print(Ia);  dataCAP.print(",");
      //dataCAP.print("Solar tracker Power   : ");
      dataCAP.print(Pa);  dataCAP.print(",");
      //dataCAP.print("AZIMUTH : ");
      dataCAP.print(AziAngle); dataCAP.print(",");
      //dataCAP.print("ALTITUDE : ");
      dataCAP.println(AltAngle);
